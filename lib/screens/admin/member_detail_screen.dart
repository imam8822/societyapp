  ...
  void _openEdit() {
    showModalBottomSheet(
      ...
      builder: (_) => EditMemberSheet(
        member: _member,
        onSaved: () {
          // Pop back to members list and let it refresh
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(children: [
                ...
                // ── Top 3 stat boxes ─────────────────────
                Row(children: [
                  _StatBox(label: 'Total Invested',
                      value: fmt.format(_member.totalInvested), color: AppTheme.primary),
                  const SizedBox(width: 10),
                  _StatBox(label: 'Months Paid',
                      value: '${_member.totalContributions}', color: const Color(0xFF2ECC71)),
                  const SizedBox(width: 10),
                  _StatBox(
                    label: 'Pending',
                    value: fmt.format(_member.pendingAmount),
                    color: _member.pendingAmount > 0 ? AppTheme.error : const Color(0xFF2ECC71),
                  ),
                ]),
                ...
                _Section(title: 'Personal Details', rows: [
                  _InfoRow(icon: Icons.phone_outlined, label: 'Mobile', value: _member.phone),
                  if (_member.email.isNotEmpty && !_member.email.contains('@society.app'))
                    _InfoRow(icon: Icons.email_outlined, label: 'Email', value: _member.email),
                  _InfoRow(icon: Icons.calendar_today_outlined, label: 'Joined',
                      value: dateFmt.format(_member.joinedDate)),
                  if (_member.referredByName != null)
                    _InfoRow(icon: Icons.person_add_outlined, label: 'Referred By',
                  ...
                  _InfoRow(
                    ...
                    valueColor: _member.isActive
                        ? const Color(0xFF2ECC71) : AppTheme.error,
                  ),
                ]),
                const SizedBox(height: 14),
                ...
                _Section(title: 'Investment Details', rows: [
                  _InfoRow(icon: Icons.savings_outlined, label: 'Pre-existing',
                      value: fmt.format(_member.preExistingInvestment)),
                  _InfoRow(icon: Icons.add_card_outlined, label: 'Paid via App',
                      value: fmt.format(_member.totalContributed)),
                  _InfoRow(icon: Icons.account_balance_wallet_outlined,
                      label: 'Total Invested',
                      value: fmt.format(_member.totalInvested), bold: true),
                  if (_member.pendingAmount > 0)
                    _InfoRow(icon: Icons.warning_amber_outlined, label: 'Pending',
                        value: fmt.format(_member.pendingAmount),
                ...
                _Section(title: 'Loan Status', rows: [
                  _InfoRow(
                    icon: Icons.verified_outlined, label: 'Loan Eligible',
                    value: _member.isEligibleForLoan ? 'Yes' : 'Not yet',
                    valueColor: _member.isEligibleForLoan
                        ? const Color(0xFF2ECC71) : AppTheme.textGrey,
                  ),
                  if (_member.hasActiveLoan) ...[
                    _InfoRow(icon: Icons.monetization_on_outlined, label: 'Active Loan',
                        value: fmt.format(_member.activeLoanAmount),
                        valueColor: AppTheme.primary),
                    _InfoRow(icon: Icons.pending_outlined, label: 'To Repay',
                        value: fmt.format(_member.loanAmountToRepay),
                        valueColor: AppTheme.error),
                    if (_member.nextInstallmentDueDate != null)
                      _InfoRow(icon: Icons.event_outlined, label: 'Next Installment',
                          value: '${fmt.format(_member.nextInstallmentAmount)} · '
                              '${dateFmt.format(_member.nextInstallmentDueDate!)}'),
                  ] else
                    const _InfoRow(icon: Icons.check_circle_outline,
                        label: 'Active Loan', value: 'None'),
                ]),
